...
function MetaTab() {
  ...
  const filtered = HERO_META.filter((h) => {
  ...
  return (
    <div>
      <Section title={`Meta hero list (${HERO_META.length})`}>
        <div style={{ fontSize: 10.5, color: "#5C6472", marginBottom: 12 }}>
          Full current roster — tiers reflect a recent patch snapshot and should be refreshed by app owners each patch, not treated as live data.
        </div>
        ...
        <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 8 }}>
          {tiers.map((t) => (
            <button
              ...
              style={{
                flexShrink: 0,
                background: tier === t ? (TIER_COLORS[t] || "#00D9C0") : "#141922",
                color: tier === t ? "#05070B" : "#8B94A3",
                border: "1px solid " + (tier === t ? (TIER_COLORS[t] || "#00D9C0") : "#262B34"),
                borderRadius: 20,
                padding: "4px 11px",
                fontSize: 11,
        ...
        <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 10, marginBottom: 4 }}>
          {roles.map((r) => (
            <button
              ...
              style={{
                flexShrink: 0,
                background: role === r ? "#00D9C0" : "#141922",
                color: role === r ? "#05070B" : "#8B94A3",
                border: "1px solid " + (role === r ? "#00D9C0" : "#262B34"),
                borderRadius: 20,
                padding: "5px 12px",
                fontSize: 11.5,
        ...
        <div style={{ fontSize: 10.5, color: "#5C6472", margin: "2px 0 6px" }}>{filtered.length} heroes</div>

        {filtered.length === 0 ? (
          <EmptyState text="No heroes match that filter." />
        ) : (
          filtered.map((h) => (
            <div key={h.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 0", borderBottom: "1px solid #1B2029" }}>
              <div
                style={{
                  width: 28,
                  height: 28,
                  borderRadius: 8,
                  background: TIER_COLORS[h.tier] + "22",
                  border: `1px solid ${TIER_COLORS[h.tier]}`,
              ...
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: 13.5, fontWeight: 700, color: "#E8ECF1" }}>{h.name}</span>
              </div>
              <div style={{ display: "flex", gap: 4 }}>
                {h.roles.map((r) => (
                  <span key={r} style={{ fontSize: 9, color: ROLE_COLORS[r], border: `1px solid ${ROLE_COLORS[r]}55`, borderRadius: 4, padding: "1px 5px", whiteSpace: "nowrap" }}>
                    {r}
                  </span>
                ))}
              </div>
